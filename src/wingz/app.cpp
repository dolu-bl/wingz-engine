...
core::StateContext App::createContext()
{
    ...
    ctx.assets = m_impl->assetManager.get();
    return ctx;
}

void App::createAssetManager()
{
    m_impl->assetManager = std::make_unique<core::AssetManager>();
}

core::AssetManager& App::assets()
{
    if (!m_impl->assetManager)
        throw std::runtime_error("AssetManager не создан. Вызовите createAssetManager() в onInit()");
